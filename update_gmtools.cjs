const fs = require('fs');
let code = fs.readFileSync('src/components/GMTools.tsx', 'utf-8');

code = code.replace(/const first = \['Kael', [^;]+;/, "const first = ['Kael', 'Zor', 'Jax', 'Vex', 'Rin', 'Nyx', 'Kaelen', 'Zephyr', 'Thane', 'Elara', 'Lyra', 'Seraphina', 'Valerius', 'Vex\\'ahlia', 'Vax\\'ildan', 'Percy', 'Keyleth', 'Pike', 'Scanlan', 'Grog', 'Taryon', 'Caleb', 'Beau', 'Fjord', 'Jester', 'Nott', 'Mollymauk', 'Caduceus', 'Yasha', 'Imogen', 'Laudna', 'Ashton', 'FCG', 'Orym', 'Fearne', 'Dorian', 'Chetney'];");
code = code.replace(/const last = \['Stormcaller', [^;]+;/, "const last = ['Stormcaller', 'Ironfist', 'Shadowbane', 'Fireforge', 'Voidwalker', 'Dawnstrider', 'Nightwhisper', 'Bloodmoon', 'Vessar', 'De Rolo', 'Trickfoot', 'Shorthalt', 'Strongjaw', 'Darrington', 'Widogast', 'Lionett', 'Stone', 'Lavorre', 'Brenatto', 'Tealeaf', 'Clay', 'Nydoorin', 'Temult'];");

// Better generation data based on active tool
const genLogic = `
      let result = '';
      if (activeTool === 'names') {
...
code = code.replace(/let result = '';[\s\S]*?setIsGenerating\(false\);/, `${genLogic}\n      setGeneratedOutput(result);\n      setIsGenerating(false);`);

fs.writeFileSync('src/components/GMTools.tsx', code);
